...
public class Tree234App {
    ...
    public static void main(String[] args) {
         ...
         *      图中上面的三个节点有子节点,底层的六个节点都是叶节点,没有子节点,2-3-4树中所有的
         * 叶节点总是在同一层上。
         *
         * 名字的含义?
         ...
         *       - 把每个3-节点转化成一个子节点和一个父节点。哪个节点变成子节点或父
         *  节点都无所谓。按照子节点是第一个数据还是第二个数据，子节点会有两个自己的子节点:W和Ⅹ或Ⅹ和Y。
         *  按照父节点是第一个数据还是第二个数据，父节点有另一个子节点:Y或W。子节点涂成红色,父节点涂成黑色。
         ...
         *        /A /B\ C \                               /A\        /C\
         *      下图中左侧展示了与上图中的2-3-4树等价的红黑树。
         *      颜色变化后得到如下图右侧所示的红黑树。现在节点40和60是黑色的,节点50是红色的。因此
         ...
         *                            50(black)           80(red)
         *                           /        \          /       \
         *                    40(red)      60(red)  75(black)   85(black)
         *                    /    \       /    \     /   \       /   \
         ...
         *
         * 2-3-4树的效率
         *      分析2-3-4树的效率比红黑树要困难,但还是可以从两者的等价性开始分析。
         *
         * 速度
         ...
         * 大致在log2(N+1)和 log2(N+1) / 2 之间。减少2-3-4树的高度可以使它的查找时间比红黑树的短
         * 一些。
         ...
         * 的数据项必须分配到父节点,父节点的新的兄弟节点,以及父节点的父节点中去。这种情况如图
         * 所示。
         *      如果父节点的父节点(叶节点的祖父节点)是满的,还是要分裂。分裂过程向上延续直到找到
         * 不满的父节点或者遇到根。如果根也是满的,就要创建一个新的根作为原来的根的父节点,插入100，如下图：
         *             |50| |                                      |50|85|
         *           /       \                         /            \             \
         *                |75|85|                                |75| |          |95| |
         *            /       \      \                          /     \        /        \
         ...
         *
         * 实现
         *      Java完整地实现2-3树留作练习，见exercise。但是,最后还是来提示一下如何处理分裂地过程
         ...
         *                                                |40| |   |50| |   |70| |   |90| |
         *                                                Child0    New      child1  Child2
         *
         *      插入73，分裂child1:
         *                |60|80|                                        |73| |
         *            /       \      \d                             /n             \n
         *        |40| |   |70|75|   |90| |                     |60| |          |80| |
         *        Child0   child1  child2                     /     \        /n       \n
         *                                                |40| |   |70| |   |75| |   |90| |
         *                                                Child0   Child1    New     Child2
         *
         *      插入93，分裂child2:
         *                |60|80|                                        |80| |
         *            /       \      \d                             /n             \n
         ...
         * - 多叉树中,节点中数据项按关键字升序排列。
         * - 2-3-4树中,所有的插入都在叶节点上,所有的叶节点在同一层上。
         * - 在2-3-4树中有三种可能的节点:2节点有1个关键字和2个子节点,3节点有2个关键
         * 字和3个子节点,4节点有3个关键字和4个子节点。
         * - 2-3-4树中没有1节点。
         * - 在2-3-4树中查找时,检查每个节点的关键字。没有找到时,如果要查找节点的关键字比
         * key0小,下一个节点就是 child0;如果要查找节点的关键字在key0和key1之间,下一个
         * 节点就是 child1;如果要查找节点的关键字在key1和key2之间,下一个节点就是 child2
         * 如果要查找节点的关键字大于key2,下一个节点就是 child3
         * - 在2-3-4树中插入需要在查找插入点的过程中,顺着树的路径向下分裂路径上每个满的节
         * 点
         * - 分裂根要创建两个新节点:分裂出另一个节点,创建一个新节点。
         * - 只有分裂根时2-3-4树的高度才会增长
         * - 2-3-4树和红-黑树之间存在一对一的对应关系
         * - 要把2-3-4树转化成红-黑树,需要把每个2节点变成黑色节点,把每个3节点变成一个
         * 黑色的父节点和一个红色的子节点,把每个4节点变成一个黑色父节点和两个红色子节
         * 点
         * - 当3-节点转化成一个父节点和一个子节点时,每个节点都可以做父节点
         * - 2-3-4树中分裂节点和在红-黑树中进行颜色变幻是一样的
         * - 红黑树中的旋转对应于转化3节点时在两种可能的(倾斜)方向间变化
         * - 2-3-4树的高度log2(N)
         * - 查找时间和高度成正比
         * - 2-3-4树很浪费空间,因为很多节点还不满一半。
         * - 2-3树类似于2-3-4树,除了它只能有1个或2个数据项以及2或3个子节点。
         * - 在2-3树中插入需要找到合适的叶节点,然后从叶节点开始向上分裂,直到找到不满的节
         * 点
         * - 外部存储的意思是在主存外面保存数据,通常是在磁盘上。
         * - 外部存储器比主存大,便宜(每字节),但是慢。
         * - 外部存储器中的数据通常需要在主存间来回传送,一次传送一块。
         * - 在外部存储器里的数据可以按关键字顺序有序排列。这样查找很快,但插入(或删除)很
         * 慢
         * - B-树是多叉树,每个节点可以有几十或上百个关键字和子节点
         * - B-树中子节点的个数总是比关键字的个数多1。
    ...
    public static void testTree234() {
        ...
        println("insert 50, 40, 60, 30, 70: ");
        theTree.insert(50, 40, 60, 30, 70);
        theTree.displayTree();

        for (Integer n : Arrays.asList(10, 20, 30, 40, 100, 55, 25)) {
            sep();
            println("insert " + n + ": ");
            theTree.insert(n);
            theTree.displayTree();
        ...
        if(found != -1)
            ...
            println("Could not find 30");

        found = theTree.find(66);
        if(found != -1)
            println("Found 66");
        else
            println("Could not find 66");
    }
